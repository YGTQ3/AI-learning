...
一个基于 React + Supabase 的 **个人学习管理工具**，帮助用户高效规划学习任务、管理资源、追踪进度，并支持社交分享与多设备同步。

![Snipaste_2025-08-28_16-56-55](README.assets/Snipaste_2025-08-28_16-56-55.png)

![Snipaste_2025-08-28_16-57-32](README.assets/Snipaste_2025-08-28_16-57-32.png)
...
- **任务管理**  
  📅 创建/编辑学习任务，设置优先级、时间、分类  
  🔄 本地缓存 + 自动同步到 Supabase 云端  
  📊 学习进度统计与可视化（图表展示）
...
- **社交互动**  
  🌐 支持资源一键分享（浏览器原生分享或复制链接）  
  👥 查看他人分享的学习资源（社交模块可扩展）

- **用户系统**  
  🔐 Supabase 认证（邮箱/密码 + 第三方登录）  
  🎨 主题切换（亮/暗/跟随系统）、通知设置  
  📱 多设备同步（记录设备信息）
...
- **前端**  
  ...
  Supabase 客户端（实时数据库 + 认证）

- **后端**  
  Supabase（PostgreSQL 数据库 + RLS 权限控制）  
  数据表：用户、任务、资源、设置、设备、同步日志等

- **工具**  
  `localStorage` 离线缓存 + 防抖同步  
  `sonner` 通知、`recharts` 数据可视化  
  `zod` 表单验证、`date-fns` 日期处理



## 🌟 项目亮点

- **离线优先**：本地操作流畅，网络恢复后自动同步  
- **模块化设计**：主应用与社交框架分离，便于复用  
- **数据驱动**：学习统计与可视化，洞察学习习惯  
- **安全认证**：Supabase RLS 策略保障用户数据隔离
